...
export default function ActiveMonitor({ metrics }: ActiveMonitorProps) {
    ...
    return (
        <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
            <h2 className="text-xl font-bold p-6 border-b border-gray-200 text-gray-800">Live Status Monitor</h2>
                ...
                {metrics.map((m) => (
                    ...
                    >
                        <div className="flex justify-between items-start mb-3">
                            <h3 className="font-bold text-gray-900 truncate pr-2">{m.metric_name}</h3>
                            <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider ${m.is_firing ? 'bg-red-600 text-white animate-pulse' : 'bg-green-600 text-white'
                                }`}>
                                {m.is_firing ? 'Firing' : 'Healthy'}
                            </span>
                        </div>
                        <div className="flex items-baseline space-x-1">
                            <span className="text-2xl font-black text-gray-900">{m.last_value}</span>
                            <span className="text-xs text-gray-500 font-medium">unit</span>
                        </div>
                        <p className="mt-2 text-[10px] text-gray-400 font-mono">
                            Last Updated: {new Date(m.updated_at).toLocaleTimeString()}
                        </p>
                    </div>
                ))}
            </div>
